...
def handle_materials():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # Generate unique filename untuk Supabase
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            unique_filename = f"{timestamp}_{filename}"
            
            # Construct path untuk Supabase bucket (e.g., materials/topik/unique_filename)
            supabase_file_path = f"materials/{topik.lower()}/{unique_filename}"
            
            # Reset file pointer ke awal sebelum upload
            file.seek(0)
            ...
            try:
                ...
                # Get public URL
                public_url = supabase_client.get_public_url(supabase_file_path)
                
                # Create material record dengan URL dari Supabase
                material = TeacherMaterial(
                    judul=judul,
                    topik=topik.lower(),
                    file_path=supabase_file_path,  # Store Supabase path, bukan local path
                    file_name=filename,
                    file_type=file_type,
                    level=level,
                ...
                print(f"   Supabase path: {supabase_file_path}")
                print(f"   Public URL: {public_url}")
                
                return jsonify({
                    'status': 'success',
                    'message': 'Material uploaded successfully to Supabase',
                    'data': {
                        **material.to_dict(),
                ...
                return jsonify({
                    'status': 'error',
                    'message': f'Failed to upload file to Supabase: {str(upload_error)}'
                }), 500
...
def handle_material_detail(material_id):
    ...
    if request.method == 'GET':
        ...
        # Delete material file dari Supabase bucket
        if material.file_path:
            try:
                deleted = supabase_client.delete_file(material.file_path)
                if deleted:
                    print(f"✅ File deleted from Supabase: {material.file_path}")
                else:
                    print(f"⚠️  Could not delete file from Supabase: {material.file_path}")
            except Exception as e:
                print(f"⚠️  Error deleting file from Supabase: {e}")
...
def download_material(material_id):
    """
    GET /api/materials/<id>/download - Download file materi dari Supabase
    """
    material = TeacherMaterial.query.get(material_id)
    ...
    try:
        ...
        file_content = supabase_client.download_file(material.file_path)
        ...
        return send_file(
            file_obj,
            as_attachment=True,
            download_name=material.file_name or f"material_{material.id}",
            mimetype=supabase_client._get_content_type(material.file_name)
        )
