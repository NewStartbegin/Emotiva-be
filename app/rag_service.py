...
class RAGService:
    ...
    def _extract_content(self, material: TeacherMaterial) -> str:
        """
        Extract text content from material (file dari Supabase atau konten field)
        """
        # If material has a file path (Supabase path), try to extract content from it
        if material.file_path:
            try:
                ...
                if file_type == 'txt':
                    ...
                    print(f"  ✅ Extracted text from {material.file_name} ({len(content)} chars) [Supabase]")
                    return content
                ...
                # Extract text from PDF files
                    ...
                    try:
                        ...
                        print(f"  ✅ Extracted text from PDF {material.file_name} ({len(content)} chars) [Supabase]")
                        return content
                    except ImportError:
                ...
                print(f"  ❌ Error reading file from Supabase {material.file_path}: {e}")
                return material.konten or ''
    ...
    def get_material_by_topik(self, topik: str, level: str = None) -> str:
        """
        Get full material by topik (dari Supabase atau konten field)
        ...
        for mat in materials:
            ...
            # Use _extract_content untuk consistency (handles both Supabase dan konten field)
            content = self._extract_content(mat)
